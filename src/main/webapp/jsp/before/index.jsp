					...
					<div class="top10List clearfix">
						<c:forEach items="${salelist }" var="sg" varStatus="status">
							<ul class="clearfix">
								...
								<li class="topimg">
								<a href="/user/goodsDetail?id=${sg.id }" target="_blank">
									<img class="samllimg" src="/pic/${sg.gpicture}" /></a></li>
								<li class="iteration1">
								<a href="/user/goodsDetail?id=${sg.id }" target="_blank">${sg.gname }</a><br />
						...
						<div class="box_1">
							...
							<div class="post_list ared">
								...
								<c:forEach items="${noticelist}" var="nt">
									<li><a href="javascript:openNotice('/selectANotice?id=${nt.id }');">${nt.ntile }</a></li>
								</c:forEach>
								</ul>
							</div>
						</div>
								...
								<c:forEach items="${lastedlist }" var="sg">
									<li>
										<div>
											<p class="pic">
												<a href="/user/goodsDetail?id=${sg.id }" target="_blank">
												<img src="/pic/${sg.gpicture}" /></a>
											</p>
											<p class="wz">
												<strong><a href="/user/goodsDetail?id=${sg.id }" target="_blank">${sg.gname }</a></strong>
